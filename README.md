...
- Example : with deit_base, fine-tune ImageNet-1k from pre-trained model (with ExFractalDB-21k), 4 GPUs (Batch Size = 64×4 = 256)
    ...
    ```bash
    $ python finetune.py path_to_cifar10 \
        --model deit_base_patch16_224 --experiment ver2 \
        ...
        --drop-path 0.1 --reprob 0.25 -j 2 \
        --output ./result/finetune \
        --pretrained-path exfractal_21k_base.pth.tar\
        --amp
    ```
